...
class Paymentintentcontroller extends GetxController {
  ...
  sendTheData(
    ...
    if ((isChecked1.isTrue && isUpi) || (isUpiPhonepe && isChecked2.isTrue)) {
      ...
      if (await canLaunchUrl(Uri.parse(url)) &&
        ...
        final _paymentStarted = await launchUrl(Uri.parse(url));
    ...
    Platform.isIOS
        ? fetchnew_HASH(Amount, customerId, name_org, phone)
        : paymentInistiate(Amount, name, name_org, order_id, hash, phone);
  }
  ...
  paymentInistiate(
    ...
    open(params, Get.context).then((value) {
      if (value != null) {
        CheckThePayment(order_id, orgname ?? "");
      } else {}
    });
  }

  CheckThePayment(String? orderid, String name) async {
