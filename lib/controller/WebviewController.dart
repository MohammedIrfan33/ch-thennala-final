...
class Webviewcontroller extends GetxController {
  ...
  void openCheckout(String orderid) {
    Map<String, dynamic> options = {
      ...
      'prefill': {
        'contact': '${number}',
        'email': '${number}@chcenterthennala.in',
      },
      'external': {
        'wallets': ['paytm'],
      },
  ...
  getGatewaydetected(
    ...
    bool isUpi = await canLaunchUrl(
      Uri.parse(
        "gpay://upi/pay?pa=chmuhammedkoyamem697890@fbl&pn=CH+MUHAMMED+KOYA+MEMORIAL+CHARITABLE+CENTRE&mc=8398&tr=FBLOMNI405324184PAY2907374559031741&tn=Test&am=2.00&cu=INR",
      ),
    );
    bool isUpiPhonepe = await canLaunchUrl(
      Uri.parse(
        "phonepe://pay?pa=chmuhammedkoyamem697890@fbl&pn=CH+MUHAMMED+KOYA+MEMORIAL+CHARITABLE+CENTRE&mc=8398&tr=FBLOMNI405324184PAY2907374559031741&tn=Test&am=2.00&cu=INR",
      ),
    );
    ...
    Get.off(
      Paymentintent(
        ...
        totalmaount: Amount ?? "",
        name: name ?? "",
        nameOrg: orderBody!["customer"] ?? "",
        hash: hash ?? "",
        order_id: order_id ?? "",
        count: coount ?? 0,
        isUpi: isUpi,
        isUpiPhonepe: isUpiPhonepe,
        phone: orderBody!["mobile"] ?? "",
      ),
    );
    ...
    //       showthewebview(true);
    //     }else{
    //       final params = getParams(Amount,name??orderBody!["customer"],number,order_id,hash); // Get parameters from hash_generator
    //       print("Params => " + params.toString());
    //
    //
    //       // Initiate payment
  ...
  transationupdate(String id, String? tdi) async {
    ...
    final response = await post(
      Uri.parse(Updatetransation),
      body: {'id': id, 'Transid': tdi.isNull ? "0" : tdi, 'Received': Amount},
    );
    //isLoading(false);
    // print(">>>>>>>>>>>>>>>>>>>>>>> details  save${response.body}");
